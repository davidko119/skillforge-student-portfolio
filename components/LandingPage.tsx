  ...
  return (
    <div className="border-b border-white/5 last:border-0 group">
      ...
      >
        ...
        <div className={`p-2 rounded-full transition-all ${isOpen ? 'bg-red-600 text-white rotate-180' : 'bg-white/5 text-zinc-500'}`}>
          <ChevronDown size={20} />
        </div>
      </button>
...
const FeatureCard = ({ title, desc, icon, delay }: { title: string, desc: string, icon: React.ReactNode, delay: string }) => (
  <div className={`group relative p-10 bg-zinc-950 border border-white/5 rounded-[2.5rem] hover:bg-zinc-900/50 transition-all duration-700 animate-in fade-in slide-in-from-bottom-10 ${delay}`}>
    <div className="absolute -inset-px bg-gradient-to-br from-red-600/20 to-transparent rounded-[2.5rem] opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
    <div className="relative z-10">
      <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center mb-8 group-hover:scale-110 group-hover:bg-red-600/10 group-hover:text-red-500 transition-all duration-500 text-zinc-400">
        {icon}
      </div>
    ...
    {/* Glow shadow effect */}
    <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-red-600/5 blur-[80px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
  </div>
);
...
const LandingPage: React.FC<LandingPageProps> = ({ onStart }) => {
  ...
  return (
    <div className="bg-[#030303] min-h-screen text-white selection:bg-red-500 selection:text-white font-inter overflow-x-hidden">
      {/* Background Lighting Blobs */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-red-600/10 blur-[180px] rounded-full animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-rose-600/10 blur-[180px] rounded-full animate-pulse [animation-delay:2s]" />
      </div>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 border-b ${scrolled ? 'bg-black/60 backdrop-blur-2xl border-white/10 h-20' : 'bg-transparent border-transparent h-28'}`}>
        <div className="max-w-7xl mx-auto px-8 h-full flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">
            <a href="#features" className="hover:text-red-500 transition-colors">Vlastnosti</a>
            <a href="#preview" className="hover:text-red-500 transition-colors">Ukážka</a>
            <a href="#pricing" className="hover:text-red-500 transition-colors">Cenník</a>
            <a href="#faq" className="hover:text-red-500 transition-colors">Otázky</a>
          </div>
          <button
            onClick={onStart}
            className="px-8 py-3 bg-white text-black font-black rounded-full hover:bg-red-600 hover:text-white hover:scale-105 transition-all text-[11px] uppercase tracking-widest shadow-xl"
          >
            Vstúpiť
          </button>
        </div>
      ...
      <section className="relative pt-64 pb-32 px-8">
        <div className="max-w-6xl mx-auto text-center space-y-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-red-500/20 bg-red-500/5 text-red-400 text-[10px] font-black uppercase tracking-[0.4em] animate-in fade-in slide-in-from-top-4 duration-1000">
          ...
          <h1 className="text-6xl md:text-8xl lg:text-9xl font-black font-jakarta tracking-tighter leading-[0.85] animate-in fade-in slide-in-from-bottom-8 duration-1000">
            Buduj budúcnosť. <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 via-rose-400 to-red-500">Buď digitálny.</span>
          </h1>
          ...
          <div className="pt-8 flex flex-col sm:flex-row items-center justify-center gap-6 animate-in fade-in slide-in-from-bottom-16 duration-1000">
            <button
              onClick={onStart}
              className="px-12 py-6 bg-red-600 text-white font-black rounded-[2.5rem] hover:scale-105 hover:shadow-[0_0_60px_rgba(220,38,38,0.5)] transition-all text-xl flex items-center gap-3 group uppercase tracking-widest"
            >
      ...
      <section id="features" className="py-32 px-8 max-w-7xl mx-auto">
        <div className="text-center mb-24 space-y-6">
          <p className="text-[10px] font-black uppercase tracking-[0.5em] text-red-500">Možnosti</p>
      ...
      <footer className="py-48 px-8 relative overflow-hidden bg-black border-t border-white/5">
        <div className="max-w-7xl mx-auto text-center space-y-16">
          <h2 className="text-6xl md:text-9xl font-black font-jakarta tracking-tighter leading-none">Začni svoju <br /> <span className="text-red-500">cestu dnes.</span></h2>
          <button
            onClick={onStart}
