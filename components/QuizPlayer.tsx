            ...
            <div className="space-y-4">
              {currentQuestion.options.map((option, idx) => (
                <button
                  ...
                  className={`w-full p-6 text-left rounded-2xl border transition-all flex items-center justify-between group ${selectedAnswers[currentStep] === idx
                      ? 'bg-red-500/10 border-red-500 text-red-500'
                      : 'bg-zinc-900 border-white/5 hover:border-white/20 text-zinc-400 hover:text-white'
                    }`}
                >
                  <span className="font-medium text-lg">{option}</span>
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${selectedAnswers[currentStep] === idx ? 'border-red-500' : 'border-zinc-700 group-hover:border-zinc-500'
                    }`}>
                    {selectedAnswers[currentStep] === idx && <div className="w-3 h-3 bg-red-500 rounded-full" />}
                  </div>
                </button>
              ))}
            </div>
