      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {quizzes.map((quiz) => {
          ...
          return (
            <div
              key={quiz.id}
              className={`group border rounded-[2.5rem] p-8 transition-all hover:scale-[1.02] ${isCompleted
                  ? 'border-green-500/20 bg-green-500/[0.02]'
                  : `${isDark ? 'bg-zinc-950 border-white/10 hover:border-red-500/30' : 'bg-zinc-50 border-black/5 shadow-sm hover:border-red-600/30'}`
                }`}
            >
              ...
              <button
                ...
                className={`w-full py-3 rounded-xl flex items-center justify-center gap-2 font-black text-xs uppercase tracking-widest transition-all ${isCompleted
