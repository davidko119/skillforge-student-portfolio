...
const SEARCH_MESSAGES = [
  "Iniciujem Apify Scraper...",
  "Analyzujem kľúčové slová profilu...",
  "Prehľadávam Google Search Results...",
  "Filtrujem relevantné príležitosti...",
  "Sťahujem dáta z datasetu...",
  "Finálne spracovanie..."
];
...
const CalendarView: React.FC<CalendarViewProps> = ({ opportunities: initialOpps, user, setUser, theme }) => {
  ...
  const fetchLiveOpportunities = async (prefs: UserPreferences) => {
    ...
    // Simulate some initial progress while waiting for Apify
    const progressInterval = setInterval(() => {
      setSearchProgress(prev => prev < 90 ? prev + 5 : prev);
    }, 1000);
    ...
    try {
      // Use the Apify service instead of Google GenAI
      const { triggerScrape } = await import('../apifyService');
      const parsed = await triggerScrape(prefs);
  ...
  useEffect(() => {
    ...
    if (lastResults) {
      ...
      // If we auto-fetch on load, we consider it a search
      // fetchLiveOpportunities(user.preferences); // Verify if we want to auto trigger on load
    }
  }, []);
  ...
  const displayOpps = hasSearched ? liveOpps : initialOpps;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-24">
            {displayOpps.length > 0 ? (
              ...
              <div className="col-span-full py-20 text-center space-y-4">
                <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto ${isDark ? 'bg-zinc-900' : 'bg-zinc-100'}`}>
                  <Search className="text-zinc-500" size={32} />
                </div>
                <h3 className={`text-xl font-black ${isDark ? 'text-white' : 'text-black'}`}>Žiadne výsledky</h3>
                <p className="text-zinc-500 text-sm font-bold">Skús zmeniť filtre alebo vyhľadávanie.</p>
              </div>
            )}
          </div>
        )}
