    ...
    <div className="p-8 max-w-4xl mx-auto pb-32">
      <header className="mb-12 flex justify-between items-end">
        ...
        <button
          ...
          className="px-8 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-2xl flex items-center gap-2 transition-all transform active:scale-95 shadow-lg shadow-red-600/20"
        >
      ...
      <div className="space-y-8">
        <section className="bg-zinc-900/50 border border-white/10 rounded-[2.5rem] p-10 space-y-6">
          <div>
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-red-500 text-xl font-bold"
              value={title}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-zinc-950 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-red-500 text-zinc-400"
              rows={3}
              value={description}
        ...
        {questions.map((q, qIdx) => (
          <div key={q.id} className="bg-zinc-900/30 border border-white/5 rounded-[2.5rem] p-10 space-y-8 group animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="space-y-4">
              ...
              <input
                type="text"
                className="w-full bg-transparent border-b-2 border-white/10 py-3 text-xl font-medium focus:border-red-500 outline-none transition-colors"
              ...
              {q.options?.map((option, oIdx) => (
                <div key={oIdx} className="flex items-center gap-3 group/opt">
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${q.correctIndex === oIdx ? 'border-green-500 bg-green-500/10' : 'border-zinc-700'
                      }`}
                  >
                  ...
                  <input
                    type="text"
                    className={`flex-1 bg-zinc-950 border border-white/5 rounded-xl px-4 py-3 outline-none focus:border-red-500 text-sm ${q.correctIndex === oIdx ? 'ring-1 ring-green-500/50' : ''
                      }`}
        ...
        <button
          onClick={addQuestion}
          className="w-full py-10 border-2 border-dashed border-white/5 rounded-[2.5rem] flex flex-col items-center justify-center text-zinc-500 hover:text-red-500 hover:border-red-500/20 hover:bg-red-500/[0.02] transition-all group"
        >
          <div className="p-4 bg-zinc-900 rounded-2xl mb-4 group-hover:bg-red-500/10 transition-colors">
            <Plus size={32} />
          </div>
